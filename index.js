// Function para levar o usuario para o contato quando clicar no botão
document.getElementById('Botao').addEventListener('click', function() {
    window.location.href = 'https://tngestao.com.br/Contato/index.html#meuFormulario';
});
  



// Function para o menu hamburguer - Menu responsivo
let show = false;

const menuSection = document.querySelector(".menu-section");
const menuToggle = menuSection.querySelector(".menu-toggle");

menuToggle.addEventListener("click", () => {
    // Inverter o valor de "show"
    show = !show;

    // Ajustar a propriedade "overflow" do body
    document.body.style.overflow = show ? "hidden" : "auto";

    // Selecionar os elementos
    var navElement = document.querySelector('.nav');
    var navHead = document.querySelector('.nav_head');
    var toggleElement = document.querySelector('.menu-toggle');

    // Ajustar as classes e propriedades com base no valor de "show"
    if (show) {
        // Se show for true, mostrar .nav e alterar flex-direction para column
        navElement.style.display = 'block';
        navHead.style.flexDirection = 'column';
        toggleElement.style.marginRight = '35px';
        toggleElement.style.marginTop = '20px';
    } else {
        // Se show for false, ocultar .nav e restaurar flex-direction para o valor padrão (ou conforme necessário)
        navElement.style.display = 'none';
        navHead.style.flexDirection = 'row'; // Isso remove a propriedade flex-direction para que ela use o valor padrão
        toggleElement.style.marginRight = '15px';
        toggleElement.style.marginTop = '0';
    }

    // Para voltar a barra de menu de volta quando a resolução for maior que 768
    if (window.innerWidth > 840) {
        navElement.style.display = 'block';
    }

    // Adicionar ou remover a classe "on" na "menu-section"
    menuSection.classList.toggle("on", show);
});




// Adicione um ouvinte de evento para verificar a largura da tela e ajustar o texto do botão
function ajustarTextoDoBotao() {
    var meuBotao = document.getElementById('Botao');
    if (window.innerWidth <= 768) {
        meuBotao.textContent = "Agende sua reunião";
    } else {
        meuBotao.textContent = "Agende sua reunião agora mesmo";
    }
}

// Adicione ouvinte de evento para verificar a largura da tela e ajustar o texto do botão
window.addEventListener('resize', ajustarTextoDoBotao);

// Chame a função no carregamento da página para garantir que o texto seja ajustado inicialmente
window.addEventListener('load', ajustarTextoDoBotao);